#!/usr/bin/env bash
# R10 paralelo: discovery com walk-forward + bootstrap + janela por ticker.
# So trilha swing (fast foi descartado pelo R9).
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-reports/r10-${RUN_ID}}"
LOG_PATH="$OUT_DIR/r10.log"
...
MIN_MINING_WIN="${MIN_MINING_WIN:-75}"
MIN_MINING_PF="${MIN_MINING_PF:-3.0}"
MIN_WILSON="${MIN_WILSON:-70}"
MAX_BINOM_M="${MAX_BINOM_M:-0.001}"

# Gate Walk-forward
WF_MIN_TRADES="${WF_MIN_TRADES:-3}"
WF_MIN_WIN="${WF_MIN_WIN:-70}"

# Gates B
MIN_VAL_TRADES="${MIN_VAL_TRADES:-15}"
MIN_VAL_WIN="${MIN_VAL_WIN:-75}"
MIN_VAL_AVG="${MIN_VAL_AVG:-4.0}"
...
echo "==========================================" | tee "$LOG_PATH"
echo "R10 PARALELO - Walk-Forward + Bootstrap" | tee -a "$LOG_PATH"
echo "==========================================" | tee -a "$LOG_PATH"
...
echo "oos        = $OOS_START a $OOS_END (jamais usado para filtrar)" | tee -a "$LOG_PATH"
...
echo "gate_WF    = win>=$WF_MIN_WIN% em cada janela com >=$WF_MIN_TRADES trades (min 2 de 3 janelas validas)" | tee -a "$LOG_PATH"
...
echo "gate_Boot  = bootstrap CI95 lower bound do avg return mining > $MIN_BOOTSTRAP_LOWER%" | tee -a "$LOG_PATH"
...
batches_prefix="$TMP_DIR/swing-batch"
...
for i in $(seq 0 $((WORKERS - 1))); do
  ...
  out_csv="$TMP_DIR/r10-mining-$i.csv"
  log_file="$TMP_DIR/worker-$i.log"
  "$PYTHON" R10/tools/r10_engine.py \
...
# Concat CSVs
final_csv="$OUT_DIR/r10-mining.csv"
first=1
for i in $(seq 0 $((WORKERS - 1))); do
  part="$TMP_DIR/r10-mining-$i.csv"
  [[ ! -f "$part" ]] && continue
  if [[ $first -eq 1 ]]; then
    cat "$part" > "$final_csv"
    first=0
...
echo "==========================================" | tee -a "$LOG_PATH"
"$PYTHON" R10/tools/validate_r10.py \
  --input-csv "$final_csv" \
  --output-csv "$OUT_DIR/r10-validated.csv" \
...
echo "==========================================" | tee -a "$LOG_PATH"
"$PYTHON" R10/tools/build_r10_final.py \
  --validated-csv "$OUT_DIR/r10-validated.csv" \
  --output-dir "$OUT_DIR" \
  --mining-start "$MINING_START" \
  --mw1-end "$MW1_END" \
  --mw2-end "$MW2_END" \
...
echo "" | tee -a "$LOG_PATH"
echo "DONE - R10 finalizado" | tee -a "$LOG_PATH"
