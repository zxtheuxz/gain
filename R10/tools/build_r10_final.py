"""Gera CSVs e MDs finais R10 a partir do CSV validado.

- r10-validated-slim.csv: todos os validados sem retornos brutos
- r10-final.csv: melhor setup por ticker (top 3)
- r10-signals-ready.csv: idem
- r10-report.md: top 30 humano com walk-forward
...
def _write_report(
    ...
    lines = [
        ...
        f"- Out-of-sample (informativo): {gates['oos_start']} a {gates['oos_end']}",
        "",
        ...
        f"- **Gate WF (walk-forward):** win >= {gates['wf_min_win']}% em CADA sub-janela "
        f"com >= {gates['wf_min_trades']} trades; tem que existir em >= 2 das 3 janelas",
        ...
        f"- **Gate C (FDR):** Benjamini-Hochberg q-value < {gates['max_fdr_q']}",
    ...
    for idx, r in enumerate(top, start=1):
        ...
        # extrai os percentuais da string "PASSOU 3/3 (95%/91%/88%)"
...
def _write_oos_report(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "",
        f"- Setups validados (passou TODOS os gates): **{len(rows)}**",
...
def main() -> int:
    ...
    parser.add_argument("--min-mining-win", type=float, default=75.0)
    parser.add_argument("--min-mining-pf", type=float, default=3.0)
    parser.add_argument("--min-wilson", type=float, default=70.0)
    parser.add_argument("--max-binom-m", type=float, default=0.001)
    parser.add_argument("--wf-min-trades", type=int, default=3)
    parser.add_argument("--wf-min-win", type=float, default=70.0)
    ...
    print(f"R10 build | validated rows: {len(rows)}")

    slim = [_strip_returns(r) for r in rows]
    _write_csv(output_dir / "r10-validated-slim.csv", slim, SLIM_FIELDS[1:])

    best = _best_per_ticker(rows, args.top_per_ticker)
    _write_csv(output_dir / "r10-final.csv", best, SLIM_FIELDS)
    _write_csv(output_dir / "r10-signals-ready.csv", best, SLIM_FIELDS)
    ...
    _write_report(output_dir / "r10-report.md", rows, best, gates)
    _write_oos_report(output_dir / "r10-oos-report.md", rows)
    print(f"R10 build DONE | final csv: r10-final.csv ({len(best)} setups, {len({r['ticker'] for r in best})} tickers)")
    return 0
