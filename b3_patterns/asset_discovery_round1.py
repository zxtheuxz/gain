...
FEATURE_LIBRARY: list[AssetDiscoveryFeature] = [
    ...
    # -- R6 novas features (tendencia longo prazo, calendario, multi-candle) --
    AssetDiscoveryFeature("close_vs_sma200", "Fechamento vs MM200"),
    AssetDiscoveryFeature("close_vs_ema200", "Fechamento vs EMA200"),
    AssetDiscoveryFeature("sma50_slope_20d", "Inclinacao da MM50 em 20D"),
    AssetDiscoveryFeature("day_of_month_bucket", "Dia do mes (bucket)"),
    AssetDiscoveryFeature("month_of_year", "Mes do ano (trimestre)"),
    AssetDiscoveryFeature("days_to_options_expiry", "Dias ate vencimento de opcoes B3"),
    AssetDiscoveryFeature("days_since_options_expiry", "Dias desde vencimento de opcoes B3"),
    AssetDiscoveryFeature("inside_bar_prev", "Candle de referencia em inside bar"),
    AssetDiscoveryFeature("engulfing_prev", "Engolfo do candle de referencia"),
    AssetDiscoveryFeature("gap_filled_prev", "Gap do candle de referencia preenchido"),
]
...
def _bucket_day_of_month(value: int) -> tuple[str, str]:
    if value <= 5:
        return "start", "inicio do mes"
    if value <= 15:
        return "first_half", "primeira metade"
    if value <= 25:
        return "second_half", "segunda metade"
    return "end", "fim do mes"


def _bucket_month_of_year(value: int) -> tuple[str, str]:
    if value <= 3:
        return "q1", "primeiro trimestre"
    if value <= 6:
        return "q2", "segundo trimestre"
    if value <= 9:
        return "q3", "terceiro trimestre"
    return "q4", "quarto trimestre"


def _bucket_days_to_expiry(value: int) -> tuple[str, str]:
    if value <= 3:
        return "near", "vencimento proximo"
    if value <= 10:
        return "mid", "vencimento medio"
    return "far", "vencimento distante"


def _bucket_days_since_expiry(value: int) -> tuple[str, str]:
    if value <= 3:
        return "fresh", "pos-vencimento recente"
    if value <= 10:
        return "settled", "ja distante do vencimento"
    return "far", "muito apos vencimento"
...
def _options_expiry_distance(trade_day: "date") -> tuple[int, int]:
    """Retorna (days_to_next_expiry, days_since_last_expiry) usando 3a sexta do mes."""
    current_expiry = _third_friday(trade_day.year, trade_day.month)
    if trade_day <= current_expiry:
        days_to = (current_expiry - trade_day).days
        prev_month = trade_day.month - 1 if trade_day.month > 1 else 12
        prev_year = trade_day.year if trade_day.month > 1 else trade_day.year - 1
        prev_expiry = _third_friday(prev_year, prev_month)
        days_since = (trade_day - prev_expiry).days
    else:
        next_month = trade_day.month + 1 if trade_day.month < 12 else 1
        next_year = trade_day.year if trade_day.month < 12 else trade_day.year + 1
        next_expiry = _third_friday(next_year, next_month)
        days_to = (next_expiry - trade_day).days
        days_since = (trade_day - current_expiry).days
    return days_to, days_since


def _detect_inside_bar(
    high_ref: float, low_ref: float, high_prev: float, low_prev: float
) -> tuple[str, str]:
    if high_ref <= high_prev and low_ref >= low_prev:
        return "inside", "candle interno"
    return "outside", "candle nao interno"


def _detect_engulfing(
    open_ref: float, close_ref: float, open_prev: float, close_prev: float
) -> tuple[str, str]:
    ref_up = close_ref > open_ref
    prev_up = close_prev > open_prev
    if ref_up and not prev_up and close_ref >= open_prev and open_ref <= close_prev:
        return "bullish", "engolfo de alta"
    if not ref_up and prev_up and close_ref <= open_prev and open_ref >= close_prev:
        return "bearish", "engolfo de baixa"
    return "none", "sem engolfo"


def _detect_gap_filled(
    open_ref: float, high_ref: float, low_ref: float, close_prev: float
) -> tuple[str, str]:
    if close_prev <= 0:
        return "no_gap", "sem gap"
    gap_pct = ((open_ref / close_prev) - 1.0) * 100.0
    if gap_pct > 0.5:
        if low_ref <= close_prev:
            return "gap_up_filled", "gap de alta preenchido"
        return "gap_up_open", "gap de alta nao preenchido"
    if gap_pct < -0.5:
        if high_ref >= close_prev:
            return "gap_dn_filled", "gap de baixa preenchido"
        return "gap_dn_open", "gap de baixa nao preenchido"
    return "no_gap", "sem gap relevante"
...
def _build_feature_states(
    *,
    ...
    ema200_values: list[float] | None = None,
    ...
    # -- R6 novas features --
    sma_200 = _window_average(close_prefix_sums, anchor_idx, 200)
    if sma_200 is not None and sma_200 > 0:
        states["close_vs_sma200"] = _bucket_signed_distance(((anchor_close / sma_200) - 1.0) * 100.0)
    if ema200_values is not None and anchor_idx >= 199 and ema200_values[anchor_idx] > 0:
        states["close_vs_ema200"] = _bucket_signed_distance(((anchor_close / ema200_values[anchor_idx]) - 1.0) * 100.0)

    sma_50_prev20 = _window_average(close_prefix_sums, anchor_idx - 20, 50) if anchor_idx >= 69 else None
    if sma_50 is not None and sma_50_prev20 is not None and sma_50_prev20 > 0:
        states["sma50_slope_20d"] = _bucket_signed_distance(((sma_50 / sma_50_prev20) - 1.0) * 100.0)

    if trade_dates is not None and idx < len(trade_dates):
        try:
            trade_day_iso = date.fromisoformat(trade_dates[idx])
            states["day_of_month_bucket"] = _bucket_day_of_month(trade_day_iso.day)
            states["month_of_year"] = _bucket_month_of_year(trade_day_iso.month)
            days_to_exp, days_since_exp = _options_expiry_distance(trade_day_iso)
            states["days_to_options_expiry"] = _bucket_days_to_expiry(days_to_exp)
            states["days_since_options_expiry"] = _bucket_days_since_expiry(days_since_exp)
        except (ValueError, IndexError):
            pass

    if anchor_idx >= 1:
        prev_high = highs[anchor_idx - 1]
        prev_low = lows[anchor_idx - 1]
        prev_open = opens[anchor_idx - 1]
        prev_close = closes[anchor_idx - 1]
        if prev_high > 0 and prev_low > 0:
            states["inside_bar_prev"] = _detect_inside_bar(anchor_high, anchor_low, prev_high, prev_low)
        if prev_open > 0 and prev_close > 0:
            states["engulfing_prev"] = _detect_engulfing(anchor_open, anchor_close, prev_open, prev_close)
        if prev_close > 0:
            states["gap_filled_prev"] = _detect_gap_filled(anchor_open, anchor_high, anchor_low, prev_close)
